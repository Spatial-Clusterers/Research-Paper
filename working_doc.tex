...
Psuedocode for the two main functions of the algorithms, \texttt{DBSCAN} and \texttt{ExpandCluster}, are provided below \cite{dbscan}:
...
The results of the empirical evaluation correlated well with the asymptotic time complexity with respect to each algorithm, with the exception of \texttt{DBSCAN}. The author proposes further research into the implementation of the algorithm, specifically as to whether the function \texttt{dbscan} is actually using the R*-tree. The indexing structure is used to calculate $\epsilon$, but upon further review it may be revealed the same structure is not being used within the algorithm itself. Furthermore, \texttt{DBSCAN} is unable to handle clustering of data with a dimensionality greater than 2, and therefore suffering the curse of dimensionality. \cite{}
