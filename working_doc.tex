...
\end{itemize}
\vspace{1 mm}
The algorithms were evaluated using a data set from the National Oceanic and Atmospheric Administration (NOAA), which was comprised of hail storm data from 1955 - 2011. The evaluation consisted of running the algorithms on steadily reduced subsets of the NOAA data set, and recording their run time for each subset. The theoretical growth rates of the different algorithms do indeed line up with their real world growth rates.
...
\caption{BIRCH}
\begin{algorithmic}[1]
  \Procedure{BirchTreeBuilder}{}\Comment{Phase 1}
    ...
    \For{$i\gets 1, n$}
      ...
      \If{$i$ closest to current node}
        ...
        \Else
        \State{check $i$ against node's children}
      \EndIf
    \EndFor
  \EndProcedure
\end{algorithmic}
...
\begin{axis}[
    ...
    width=9cm,
    xlabel={seconds},
    ylabel={data points in thousands},
    grid=major,
  ]
  \addplot coordinates {
    (150000,25.030436)
